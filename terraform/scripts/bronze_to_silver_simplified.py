...
from pyspark.sql.functions import (
    col, concat, count, current_date, current_timestamp, date_format,
    dayofmonth, dayofweek, lit, month, quarter, to_date, to_timestamp,
    weekofyear, when, year, max as sql_max, sum as sql_sum
)
from pyspark.sql.types import BooleanType, DoubleType, LongType, StringType, StructField, StructType

args = getResolvedOptions(
    sys.argv,
    ["JOB_NAME", "S3_BUCKET", "BRONZE_DATABASE", "SILVER_DATABASE", "incremental", "triggered_by"]
)
...
def convert_nano_to_timestamp(df, timestamp_cols):
    """Converte colunas timestamp de nanosegundos para timestamp"""
    for col_name in timestamp_cols:
        if col_name in df.columns:
            df = df.withColumn(col_name, to_timestamp((col(col_name) / 1000000000).cast("double")))
    return df

def add_silver_metadata(df):
    """Adiciona metadados da camada Silver"""
    return df.withColumn("silver_created_at", current_timestamp()).withColumn("silver_execution_date", lit(EXECUTION_DATE)).withColumn("triggered_by", lit(TRIGGERED_BY))

def load_bronze_table(table_name):
    """Carrega tabela bronze"""
    df = glueContext.create_dynamic_frame.from_catalog(database=BRONZE_DATABASE, table_name=table_name).toDF()
...
def save_to_silver(df, table_name, partition_keys=[]):
    ...
    glueContext.write_dynamic_frame.from_options(
        frame=dyf,
        connection_type="s3",
        connection_options={"path": f"{S3_SILVER_PATH}{table_name}/", "partitionKeys": partition_keys},
        format="parquet",
...
# DIM_FORNECEDORES
dim_fornecedores = add_silver_metadata(fornecedores_bronze.select(
...
# DIM_CATEGORIAS
dim_categorias = add_silver_metadata(categorias_bronze.select(
...
# DIM_PRODUTOS
dim_produtos = add_silver_metadata(produtos_bronze.alias("p")
...
# DIM_CLIENTES
dim_clientes = add_silver_metadata(clientes_bronze.select(
    ...
    when(col("data_nascimento").isNotNull(), year(current_date()) - year(col("data_nascimento"))).otherwise(lit(None)).alias("cliente_idade"),
    col("genero").alias("cliente_genero"),
    col("criado_em").alias("cliente_criado_em"),
).distinct())

# DIM_ENDERECOS
dim_enderecos = add_silver_metadata(enderecos_bronze.select(
...
# DIM_TEMPO
dim_tempo = add_silver_metadata(vendas_bronze.select(
...
# FACTS_VENDAS
facts_vendas = add_silver_metadata(vendas_bronze.alias("v")
    ...
    .select(
        ...
        (col("quantidade") > 0) & (col("preco_unitario") > 0) & (col("subtotal") > 0) &
        (col("dim_cliente_id").isNotNull()) & (col("dim_produto_id").isNotNull())
    ))
